...
public:
    ...
    {
        //if (!Environment().IsTraining()) { Value().SetValue((ElemType)1.0); return; }
        ...
        m_softmaxOfRight->SetValue(*m_logSoftmaxOfRight);
        (*m_softmaxOfRight) *= m_acweight;
        m_softmaxOfRight->InplaceExp();
        int nf = m_softmaxOfRight->GetNumCols();
        int nstates = m_tmap->GetNumCols();

#ifdef _DEBUG
        //SaveMatrix(L"D:\\temp\\cntk_p_160418_log.txt", m_logSoftmaxOfRight);
        //SaveMatrix(L"D:\\temp\\cntk_p_160418.txt", m_softmaxOfRight);
#endif
        ...
        Value().AssignInnerProductOfMatrices(Input(0)->ValueFor(fr), *m_logSoftmaxOfRight);
        Value() *= (ElemType)-1.0;
        Value() += logfwscore;
#if NANCHECK
